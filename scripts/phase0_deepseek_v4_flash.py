#!/usr/bin/env python3
"""Phase-0 probe — DeepSeek V4 Flash (actor `deepseek_v4_flash`, via OpenRouter).

Independent-lab open-weight actor (DeepSeek) added as a fast replacement for the slow ~1T
kimi_k2_thinking — same cross-family-robustness role (CLAUDE.md), but a "flash" MoE. Gates whether it
can be a load-bearing faithful §3.1/§3.2 actor (a faithful actor needs a raw-disableable channel AND a
CoT-necessary task). Checks the three preconditions:
...
Needs OPENROUTER_API_KEY + HF_TOKEN (GPQA).
  ...
  uv run python scripts/phase0_deepseek_v4_flash.py                  # 8 items × 3 samples
  uv run python scripts/phase0_deepseek_v4_flash.py --n 1 --samples 1    # cheap smoke
"""
import argparse
import asyncio
