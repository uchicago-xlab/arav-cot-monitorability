#!/usr/bin/env python3
"""Phase-0 probe — Kimi K2 Thinking (actor `kimi_k2_thinking`, Moonshot via OpenRouter).

Gates whether kimi_k2_thinking can be a load-bearing faithful §3.1/§3.2 actor (CLAUDE.md: a faithful
...
If all three pass, confirm cot_access=native_raw + promote necessity in configs/models.yaml (and pin
the provider/quant). Needs OPENROUTER_API_KEY + HF_TOKEN (GPQA).

  uv run python scripts/phase0_kimi_k2.py                   # 8 items × 3 samples
  uv run python scripts/phase0_kimi_k2.py --n 1 --samples 1     # cheap smoke
"""
import argparse
import asyncio
...
async def main(args):
    ...
    if raw_ok and prefill_ok and necessity_ok:
        print("  -> PROMOTE: set necessity=confirmed + confirm cot_access=native_raw in configs/models.yaml.")
    else:
