...
FAMS = ["bignum", "iterated", "deductive", "indirection", "arithmetic"]  # arithmetic last → other 4 keep their colors
...
def grouped_bar(metric, title, ylabel, fname, label_fmt="{:.2f}"):
    ...
    x = np.arange(len(ACTORS)); w = 0.8 / len(FAMS)
    for i, fam in enumerate(FAMS):
        vals = [DATA[a].get(fam, {}).get(metric) or 0 for a in ACTORS]
        ax.bar(x + (i - (len(FAMS) - 1) / 2) * w, vals, w, color=FCOLOR[fam], label=fam)
